...
pub fn spawn_loot(
    ...
    let staff_visual = visuals.get(&game_assets.wooden_staff_visual)
        .expect("Wooden staff visual should be loaded by now!");
    
    // Spawn 2 wooden staffs in random rooms
    let staff_count = 2;
    
    info!("🪄 Spawning {} wooden staffs across {} rooms", staff_count, room_positions.len());
    ...
    for i in 0..staff_count {
        ...
        info!("  🪄 Staff {} spawned at room {:?} (world: {:?})", i + 1, room_pos, final_pos);
    }
