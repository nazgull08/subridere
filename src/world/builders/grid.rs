...
pub fn register_room(map: &mut RoomMap, pos: IVec3) {
        ...
        if map.rooms.contains_key(&neighbor_pos) {
            ...
            *door_flag = true;
            ...
            if let Some(neigh) = map.rooms.get_mut(&neighbor_pos) {
                if offset.z == -1 { neigh.wall_flags.front = false; neigh.door_flags.front = true; }
                if offset.z ==  1 { neigh.wall_flags.back  = false; neigh.door_flags.back  = true; }
                if offset.x == -1 { neigh.wall_flags.right = false; neigh.door_flags.right = true; }
                if offset.x ==  1 { neigh.wall_flags.left  = false; neigh.door_flags.left  = true; }
            }
        } else {
    ...
    map.rooms.insert(pos, RoomMetadata {
        wall_flags: wall,
        door_flags: door,
        has_light: true,
        entity: None,
