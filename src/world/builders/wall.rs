...
pub fn spawn_wall_with_door(
    ...
    spawn_panel(
        spawner, meshes, material.clone(),
        Vec3::new(side_w, wall.y, t), left,  format!("{name}_Left"),
    );
    spawn_panel(
        spawner, meshes, material.clone(),
        Vec3::new(side_w, wall.y, t), right, format!("{name}_Right"),
    );

    if top_h > 0.0 {
        spawn_panel(
            spawner, meshes, material,
            Vec3::new(door_w, top_h, t), top_center, format!("{name}_Top"),
        );
    }
}
